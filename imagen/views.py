...
def gemini_request_view(request):
    try:
        ...
        rules_geminy_text = data.get('rules_geminy_text')
        object_id = data.get('object_id')

        if not gemini_text:
            return JsonResponse({'success': False, 'error': 'Gemini request text is empty.'}, status=400)

        ai_response_text, p_tokens, c_tokens = imagen_services.get_gemini_text_response(
            prompt=gemini_text,
            rules_geminy=rules_geminy_text
        )
        ...
        obj.rules_geminy = rules_geminy_text
